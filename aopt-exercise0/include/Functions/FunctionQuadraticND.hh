...
namespace AOPT {
    ...
    public:
        ...
        inline virtual double eval_f(const Vec &_x) {
            ...
            //-------------------------------------------------------------------------------//
                    // Ensure the input vector size matches the number of unknowns
            if (_x.size() != n_) {
                std::cerr << "Warning: Input vector size does not match number of unknowns!" << std::endl;
                return 0.0; // or handle the error as you prefer
            }

            // Compute the quadratic term: 0.5 * x^T A x
            double quad_term = 0.0;
            for (int i = 0; i < n_; ++i) {
                for (int j = 0; j < n_; ++j) {
                    quad_term += 0.5 * _x[i] * A_(i, j) * _x[j];
                }
            }

            // Compute the linear term: b^T x
            double linear_term = 0.0;
            for (int i = 0; i < n_; ++i) {
                linear_term += b_[i] * _x[i];
            }

            // Combine the terms
            return quad_term + linear_term + c_;
        }
